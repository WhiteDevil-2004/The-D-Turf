...
SMTP_PASSWORD       = os.getenv('SMTP_PASSWORD') # Needs to be set in Render
...
def calculate_price(date_str):
    """Calculates price: WD=1000, WE=1200. Apply 50% discount if Day 1-10."""
    ...
    if days_since_launch <= 10:
        return base, base * 0.50 # (Original, Discounted)
    return base, base

def send_email(to_email, subject, body):
    if not SMTP_PASSWORD: return # Don't crash if password not set
    try:
        msg = MIMEMultipart()
        msg['From'] = SMTP_EMAIL
        msg['To'] = to_email
...
# ... (Login/Register/Logout routes keep them as they are) ...
...
def create_order():
    ...
    orig, disc = calculate_price(date)
    
    for t in times:
        if conn.execute('SELECT id FROM bookings WHERE date = ? AND time = ? AND status = "Confirmed"', (date, t)).fetchone():
            conn.close(); return jsonify({"error": f"Slot {t} is already booked!"}), 400
        total_amount_paise += int(disc * 100)

    try:
        order = rzp_client.order.create({"amount": total_amount_paise, "currency": "INR", "receipt": f"u{session['user_id']}"})
        for t in times:
        ...
        return jsonify({"success": True, "order_id": order['id'], "amount": total_amount_paise, "user_name": user['name'], "user_phone": user['phone']})
...
def verify_payment():
    ...
    try:
        ...
        email_body = f"New Booking Received!\n\nName: {info[0]['name']}\nPhone: {info[0]['phone']}\nDate: {info[0]['date']}\nSlots: {slots_str}\nPayment ID: {pay_id}"
        send_email(SMTP_EMAIL, "🏏 NEW TURF BOOKING!", email_body)
        
        conn.commit(); conn.close()
        return jsonify({"success": True, "booking_id": info[0]['id']})
    except: return jsonify({"success": False, "error": "Security failed"}), 400

# ... (Add the missing Login/Register routes back to the file manually or keep current) ...
...
def view_ticket(booking_id):
    ...
    if not booking or booking['status'] != 'Confirmed':
        conn.close(); return "Ticket not confirmed yet.", 403
